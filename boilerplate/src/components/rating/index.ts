...
export class CaseRating extends LitElement {
  ...
  static styles = [
    css`
      ...
      .star:hover {
        fill: var(--star-hover-fill-color, orange);
      }

      .star:focus {
        outline: none;
        fill: var(--star-focus-fill-color, orange);
      }
  ...
  render() {
    return html`
      ...
      (value) => html`
            <svg
              class="star ${value <= this.rating ? 'rated' : ''}"              
              xmlns="http://www.w3.org/2000/svg"
              viewBox="0 0 24 24"
              @click=${() => this.handleRatingClick(value)}
              tabindex="0"
              @keydown=${(event: KeyboardEvent) => this.handleStarKeyDown(event, value)}
              aria-label="${value <= this.rating ? 'Rated' : 'Not Rated'}"
            >
  ...
  handleRatingClick(value: number) {
    if (!this.hasAttribute('disabled')) {
      this.rating = value;
    }
  }

  handleStarKeyDown(event: KeyboardEvent, value: number) {
    if (event.key === 'Enter') {
